# dmzlux.pdl.local — vsftpd Configuration
### Apache web server + vsftpd FTP | Ubuntu | PRIVATE-2 (10.0.9.102)
...
rsa_cert_file=/etc/apache/ssl/www.enta.pt.crt
rsa_private_key_file=/etc/apache/ssl/www.enta.pt.key
...
- The `ubuntu` / `admin` user must have read/write access to both web roots
- Passive mode ports `40000–50000` must be allowed in the instance security group
- The SSL certificate is shared with Apache — issued by the CA on `srv.pdl.local`
